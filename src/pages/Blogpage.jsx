...
export default function BlogPage({ setPage, t }) {
  ...
  const sendEmailJS = async (subject, messageText) => {
    if (!email) {
      setSubscriptionError(blogT.newsletter.error);
      return false;
    }
    ...
    try {
      ...
      setSubscriptionError(blogT.newsletter.errorGeneric);
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  const resultMessage = () => {
    if (filtered.length === 0) return blogT.noResults;
    const shown = Math.min(visibleCount, filtered.length);
    const total = filtered.length;
    return blogT.showingResults
    ...
    <main className={styles.mainContent}>
        ...
        <FadeUp delay={0}>
          <div className={styles.pageHeader}>
            <div className={styles.pageHeaderLeft}>
              <h1 className={styles.pageHeaderTitle}>{blogT.pageTitle}</h1>
              <p className={styles.pageHeaderSub}>{blogT.pageSub}</p>
            </div>
            <div className={styles.pageHeaderRight}>
              <button className={styles.headerSubscribeBtn} onClick={scrollToNewsletter}>
                <MailIcon />
                <span>{blogT.subscribeBtn}</span>
              </button>
            </div>
          </div>
        </FadeUp>
                ...
                <div className={styles.featuredOverlay}>
                  <div className={styles.featuredOverlayContent}>
                    ...
                    <button className={styles.featuredReadBtn}>
                      {blogT.featured.readMore} <ArrowIcon />
                    </button>
                  </div>
                </div>
              </div>
        ...
        <div className={styles.articlesSection}>
          <div className={styles.articlesSectionHeader}>
            <div className={styles.sectionTitleRow}>
              ...
              <h2 className={styles.sectionTitle}>{blogT.sectionTitle}</h2>
          ...
          {filtered.length === 0 ? (
            <p className={styles.emptyState}>{blogT.noResults}</p>
          ) : (
            <>
      ...
      <FadeUp>
        <div className={styles.topicsBand}>
          ...
          <div className={styles.container}>
            <h2 className={styles.topicsTitle}>{blogT.exploreTopicsTitle}</h2>
            <p className={styles.topicsSub}>{blogT.exploreTopicsSub}</p>
            <div className={styles.topicsList}>
              {blogT.topics.map(t => (
                <button key={t.label} className={styles.topicPill}>
                  {t.label}
      ...
      <FadeUp>
        <div ref={newsletterRef} className={styles.newsletterSection}>
          <div className={styles.newsletterInner}>
            <span className={styles.newsletterEyebrow}>{blogT.newsletter.eyebrow}</span>
            <h2 className={styles.newsletterTitle}>{blogT.newsletter.title}</h2>
            <p className={styles.newsletterSub}>{blogT.newsletter.sub}</p>
            {subscriptionError && (
            ...
            {subscribed ? (
              <div className={styles.subscribedMsg}>
                <CheckIcon />
                <span>{blogT.newsletter.success}</span>
              </div>
            ) : (
              <div className={styles.subscribeForm}>
                ...
                >
                  {isLoading ? blogT.newsletter.sending : blogT.newsletter.button}
                </button>
              </div>
            )}
          </div>
