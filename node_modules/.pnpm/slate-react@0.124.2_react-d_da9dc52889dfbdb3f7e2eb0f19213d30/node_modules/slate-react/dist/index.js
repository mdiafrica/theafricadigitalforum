...
function createAndroidInputManager(_ref) {
  ...
  var performAction = function performAction() {
    ...
    if (action.at) {
      var target = slate.Location.isPoint(action.at) ? slateDom.normalizePoint(editor, action.at) : slateDom.normalizeRange(editor, action.at);
      if (!target) {
        return;
      }
...
var useAndroidInputManager = !slateDom.IS_ANDROID ? function () {
  ...
  var _useState = React.useState(function () {
      return createAndroidInputManager(_objectSpread$7({
        editor: editor
      }, options));
    }),
    _useState2 = _slicedToArray(_useState, 1),
...
var useDecorations = function useDecorations(node, parentDecorations) {
  ...
  var equalityFn = slate.Node.isText(node) ? slateDom.isTextDecorationsEqual : slateDom.isElementDecorationsEqual;
...
var useChildren = function useChildren(props) {
  ...
  var isBlock = slate.Node.isElement(node) && !editor.isInline(node);
  ...
  if (!chunking) {
    return node.children.map(function (n, i) {
      return slate.Node.isText(n) ? renderTextComponent(n, i) : renderElementComponent(n, i);
    });
  }
  ...
  return /*#__PURE__*/React.createElement(ReadOnlyContext.Provider, {
    ...
    onBlur: React.useCallback(function (event) {
      ...
      if (relatedTarget != null && slateDom.isDOMNode(relatedTarget) && ReactEditor.hasDOMNode(editor, relatedTarget)) {
        ...
        if (slate.Node.isElement(node) && !editor.isVoid(node)) {
          return;
        }
      }
    ...
    onClick: React.useCallback(function (event) {
      if (ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onClick) && slateDom.isDOMNode(event.target)) {
        ...
        if (event.detail === slateDom.TRIPLE_CLICK && path.length >= 1) {
          var blockPath = path;
          if (!(slate.Node.isElement(node) && slate.Editor.isBlock(editor, node))) {
            var _block$;
            var block = slate.Editor.above(editor, {
              match: function match(n) {
                return slate.Node.isElement(n) && slate.Editor.isBlock(editor, n);
    ...
    onDragStart: React.useCallback(function (event) {
      if (!readOnly && ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onDragStart)) {
        var node = ReactEditor.toSlateNode(editor, event.target);
        var path = ReactEditor.findPath(editor, node);
        var voidMatch = slate.Node.isElement(node) && slate.Editor.isVoid(editor, node) || slate.Editor["void"](editor, {
          at: path,
          voids: true
        });
        ...
        if (!slateDom.HAS_BEFORE_INPUT_SUPPORT) {
          ...
          if (slateDom.IS_CHROME || slateDom.IS_WEBKIT) {
            ...
            if (selection && (slateDom.Hotkeys.isDeleteBackward(nativeEvent) || slateDom.Hotkeys.isDeleteForward(nativeEvent)) && slate.Range.isCollapsed(selection)) {
              ...
              if (slate.Node.isElement(currentNode) && slate.Editor.isVoid(editor, currentNode) && (slate.Editor.isInline(editor, currentNode) || slate.Editor.isBlock(editor, currentNode))) {
                event.preventDefault();
                slate.Editor.deleteBackward(editor, {
                  unit: 'block'
                });
...
var Slate = function Slate(props) {
  ...
  var onContextChange = React.useCallback(function () {
    ...
    if (onSelectionChange && editor.operations.find(function (op) {
      return op.type === 'set_selection';
    })) {
      onSelectionChange(editor.selection);
    }
    if (onValueChange && editor.operations.find(function (op) {
      return op.type !== 'set_selection';
    })) {
