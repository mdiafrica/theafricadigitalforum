...
var useHydrateStore = (atoms, initialValues, options = {}) => {
  const values = import_react.default.useMemo(() => {
    const nextValues = [];
...
var useSyncStore = (atoms, values, {
  skipInitialValues,
  store: storeOption
} = {}) => {
  const atomEntries = import_react.default.useMemo(
    () => Object.entries(atoms),
    [atoms]
  );
  const syncAtom = import_react.default.useMemo(
    () => (0, import_jotai2.atom)(
      null,
      (_get, set, nextValues) => {
  ...
  const syncValues = (0, import_jotai2.useSetAtom)(syncAtom, { store: storeOption });
  const previousValuesRef = import_react.default.useRef(skipInitialValues);
...
var HydrateAtoms = (_a) => {
  ...
  const hydratedValues = __spreadValues(__spreadValues({}, initialValues), props);
  useHydrateStore(atoms, hydratedValues, {
    store
  });
  useSyncStore(atoms, props, {
    store,
    skipInitialValues: props
  });
...
var createAtomProvider = (storeScope, atoms, options = {}) => {
  ...
  return (_a) => {
    ...
    const [storeState, setStoreState] = import_react2.default.useState(
      () => store != null ? store : (0, import_vanilla.createStore)()
    );
    const resolvedStore = store != null ? store : storeState;
    ...
    return /* @__PURE__ */ import_react2.default.createElement(AtomStoreContext.Provider, { value: storeContext }, /* @__PURE__ */ import_react2.default.createElement(HydrateAtoms, __spreadValues({ store: resolvedStore, atoms }, props), !!Effect && /* @__PURE__ */ import_react2.default.createElement(Effect, null), children));
  };
};
...
var createAtomStore = (initialState, {
  ...
  const useAtomValueWithStore = (atomConfig, store, optionsOrScope, selector = identity, equalityFnOrDeps, deps) => {
    ...
    const selectorAtom = import_react3.default.useMemo(
      () => memoizedSelector === identity && !memoizedEqualityFn ? atomConfig : (0, import_utils2.selectAtom)(
        atomConfig,
        memoizedSelector,
        memoizedEqualityFn
      ),
...
function useStoreAtomValue(store, atom3, selector, equalityFnOrDeps, deps) {
  return store.useAtomValue(atom3, selector, equalityFnOrDeps, deps);
}
function useStoreSetAtom(store, atom3) {
  return store.useSetAtom(atom3);
}
function useStoreAtomState(store, atom3) {
  return store.useAtomState(atom3);
}
